...
def build_tool_registry(agent_row: dict) -> Tuple[ToolRegistry, List[Dict[str, Any]]]:
    ...
    Returns ``(registry, manifest)`` where manifest is the OpenAI-style tool
    schema list with a ``safety`` field appended for the UI.
    """
    registry = ToolRegistry()
    manifest: List[Dict[str, Any]] = []
    for name in _json_list(agent_row.get("tools")):
        try:
        ...
        manifest.append({
            ...
            "parameters": wrapper.signature or {"properties": {}, "required": []},
...
class PlatformAgentRuntime:
    ...
    def __init__(
        ...
        manager = FailoverLLMManager(
            model_preferences=self.persona.model_preferences,
